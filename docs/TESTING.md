...
1. **Test Database**: Create a test database
   ...
   ```bash
   createdb find_me_test
   ```

2. **Run Migrations**: Apply migrations to test database

   ```bash
   export DATABASE_URL="postgresql://postgres:postgres@localhost:5432/find_me_test?sslmode=disable"
   make migrate-up
   ```
...
3. Default: `postgresql://postgres:postgres@localhost:5432/find_me_test?sslmode=disable`
...
2. Check database exists: `psql -l | grep find_me_test`
