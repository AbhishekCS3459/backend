...
dev:
	...
	@if command -v psql > /dev/null 2>&1; then \
		for i in 1 2 3 4 5; do \
			if PGPASSWORD=postgres psql -h localhost -p 5434 -U postgres -d find_me -c "SELECT 1;" > /dev/null 2>&1; then \
...
# Create app databases in Compose Postgres (safe if they already exist)
db-create:
	@docker compose exec -T postgres psql -U postgres -tc "SELECT 1 FROM pg_database WHERE datname = 'find_me'" | grep -q 1 || \
		docker compose exec -T postgres psql -U postgres -c "CREATE DATABASE find_me;"
...
test-setup:
	...
	@docker compose exec -T postgres psql -U postgres -tc "SELECT 1 FROM pg_database WHERE datname = 'find_me_test'" | grep -q 1 || \
		docker compose exec -T postgres psql -U postgres -c "CREATE DATABASE find_me_test;" > /dev/null 2>&1 && \
	...
	$$MIGRATE_BIN -path migrations -database "postgresql://postgres:postgres@localhost:5434/find_me_test?sslmode=disable" up || \
...
redis-up: ## Start Redis container for queue testing
	...
	@docker run -d -p 6379:6379 --name find-me-backend-redis redis:7-alpine 2>/dev/null || \
		docker start find-me-backend-redis 2>/dev/null || \
...
redis-down: ## Stop Redis container
	...
	@docker stop find-me-backend-redis 2>/dev/null || echo "Redis container not running"
	@docker rm find-me-backend-redis 2>/dev/null || echo "Redis container not found"
